...
def _init():
    defaults = {
        ...
        # Grid de Cursos
        "t12_course_items": [
            {"img": "https://images.unsplash.com/photo-1542744094-3a31f272c490?auto=format&fit=crop&w=400&q=80", "cat": "Marketing Digital", "titulo": "Facebook Ads: Domine o Gerenciador", "rating": "4.8", "alunos": "12k alunos", "url": "https://www.google.com/"},
            {"img": "https://images.unsplash.com/photo-1561070791-2526d30994b5?auto=format&fit=crop&w=400&q=80", "cat": "Design", "titulo": "Adobe Illustrator: Ilustração Vetorial", "rating": "4.9", "alunos": "45k alunos", "url": "https://www.google.com/"},
            {"img": "https://images.unsplash.com/photo-1498050108023-c5249f4df085?auto=format&fit=crop&w=400&q=80", "cat": "Tecnologia", "titulo": "Introdução ao Desenvolvimento Web", "rating": "4.7", "alunos": "30k alunos", "url": "https://www.google.com/"},
            {"img": "https://images.unsplash.com/photo-1542744094-3a31f272c490?auto=format&fit=crop&w=400&q=80", "cat": "Dados", "titulo": "Excel para Negócios: Avançado", "rating": "4.9", "alunos": "18k alunos", "url": "https://www.google.com/"},
        ],
        ...
        "t12_foot_descs": [{"valor": "Transformando o futuro através da educação. <br> © 2026 Crehana Inc. Todos os direitos reservados."}],
        # Observações
        "t12_obs": [{"valor": ""}],
    }
    ...
    with col_form:
        ...
        with st.container(height=720, border=False):
            ...
            for i, cor in enumerate(st.session_state.t12_cores):
                ...
                with c1: st.session_state.t12_cores[i]["nome"] = st.text_input("Nome", cor["nome"], key=f"t12_cor_n_{i}", label_visibility="collapsed")
                with c2: st.session_state.t12_cores[i]["valor"] = st.color_picker("Cor", cor["valor"], key=f"t12_cor_v_{i}", label_visibility="collapsed")
                with c3:
            ...
            for i, logo in enumerate(st.session_state.t12_logos):
                ...
                with c1: st.session_state.t12_logos[i]["valor"] = st.text_input("Logo", logo["valor"], key=f"t12_logo_{i}", label_visibility="collapsed")
                with c2:
            ...
            if _add_btn("t12_logo_add", "＋ Adicionar logo"):
                st.session_state.t12_logos.append({"valor": "crehana"}); st.rerun()
            
            st.caption("Links do Menu *(Texto | URL)*")
            for i, link in enumerate(st.session_state.t12_nav_links):
                c1, c2, c3 = st.columns([4, 4, 1])
                with c1: st.session_state.t12_nav_links[i]["texto"] = st.text_input("Texto", link["texto"], key=f"t12_nl_t_{i}", label_visibility="collapsed")
                with c2: st.session_state.t12_nav_links[i]["url"] = st.text_input("URL", link["url"], key=f"t12_nl_u_{i}", label_visibility="collapsed")
                with c3:
                    if len(st.session_state.t12_nav_links) > 1 and _del_btn(f"t12_nl_del_{i}"):
                        st.session_state.t12_nav_links.pop(i); st.rerun()
            if _add_btn("t12_nl_add", "＋ Adicionar link"):
            ...
            for i, label in enumerate(st.session_state.t12_hero_labels):
                st.session_state.t12_hero_labels[i]["valor"] = st.text_input("Label Superior", label["valor"], key=f"t12_h_l_{i}")
            for i, t in enumerate(st.session_state.t12_hero_titulos):
                st.session_state.t12_hero_titulos[i]["valor"] = st.text_area("Título (Use <span class=\"highlight\"> para roxo)", t["valor"], key=f"t12_h_t_{i}")
            for i, d in enumerate(st.session_state.t12_hero_descs):
                st.session_state.t12_hero_descs[i]["valor"] = st.text_area("Descrição Hero", d["valor"], key=f"t12_h_d_{i}")
            for i, img in enumerate(st.session_state.t12_hero_imgs):
                st.session_state.t12_hero_imgs[i]["valor"] = st.text_input("URL Imagem Hero", img["valor"], key=f"t12_h_i_{i}")
            for i, btn in enumerate(st.session_state.t12_hero_btns):
                c1, c2 = st.columns([5, 5])
                with c1: st.session_state.t12_hero_btns[i]["texto"] = st.text_input("Texto Botão", btn["texto"], key=f"t12_hb_t_{i}")
                with c2: st.session_state.t12_hero_btns[i]["url"] = st.text_input("URL Botão", btn["url"], key=f"t12_hb_u_{i}")
            ...
            for i, perg in enumerate(st.session_state.t12_eng_perguntas):
                st.session_state.t12_eng_perguntas[i]["valor"] = st.text_input("Texto da Pergunta", perg["valor"], key=f"t12_ep_{i}")
            ...
            for i, item in enumerate(st.session_state.t12_course_items):
                with st.expander(f"Curso {i+1}: {item['titulo']}"):
                    st.session_state.t12_course_items[i]["titulo"] = st.text_input("Título do Curso", item["titulo"], key=f"t12_ci_t_{i}")
                    st.session_state.t12_course_items[i]["cat"] = st.text_input("Categoria", item["cat"], key=f"t12_ci_c_{i}")
                    st.session_state.t12_course_items[i]["rating"] = st.text_input("Avaliação (ex: 4.9)", item["rating"], key=f"t12_ci_r_{i}")
                    st.session_state.t12_course_items[i]["alunos"] = st.text_input("Qtd Alunos (ex: 12k alunos)", item["alunos"], key=f"t12_ci_a_{i}")
                    st.session_state.t12_course_items[i]["img"] = st.text_input("URL Imagem Card", item["img"], key=f"t12_ci_i_{i}")
                    st.session_state.t12_course_items[i]["url"] = st.text_input("URL do Botão", item["url"], key=f"t12_ci_u_{i}")
                    if len(st.session_state.t12_course_items) > 1 and _del_btn(f"t12_ci_del_{i}", "Remover curso"):
                        st.session_state.t12_course_items.pop(i); st.rerun()
            if _add_btn("t12_ci_add", "＋ Adicionar curso"):
                st.session_state.t12_course_items.append({"img": "", "cat": "CATEGORIA", "titulo": "NOVO CURSO", "rating": "5.0", "alunos": "0 alunos", "url": "#"}); st.rerun()
            ...
            for i, img in enumerate(st.session_state.t12_emp_imgs):
                st.session_state.t12_emp_imgs[i]["valor"] = st.text_input("URL Imagem Seção", img["valor"], key=f"t12_ei_i_{i}")
            for i, t in enumerate(st.session_state.t12_emp_titulos):
                st.session_state.t12_emp_titulos[i]["valor"] = st.text_input("Título Seção", t["valor"], key=f"t12_ei_t_{i}")
            for i, d in enumerate(st.session_state.t12_emp_descs):
                st.session_state.t12_emp_descs[i]["valor"] = st.text_area("Descrição Seção", d["valor"], key=f"t12_ei_d_{i}")
            
            st.caption("Benefícios/Features *(Checklist)*")
            for i, feat in enumerate(st.session_state.t12_emp_features):
                c1, c2 = st.columns([9, 1])
                with c1: st.session_state.t12_emp_features[i]["valor"] = st.text_input("Benefício", feat["valor"], key=f"t12_ef_v_{i}", label_visibility="collapsed")
                with c2:
            ...
            for i, btn in enumerate(st.session_state.t12_emp_btns):
                c1, c2 = st.columns([5, 5])
                with c1: st.session_state.t12_emp_btns[i]["texto"] = st.text_input("Texto Botão Empresas", btn["texto"], key=f"t12_eb_t_{i}")
                with c2: st.session_state.t12_emp_btns[i]["url"] = st.text_input("URL Botão Empresas", btn["url"], key=f"t12_eb_u_{i}")
            ...
            for i, logo in enumerate(st.session_state.t12_foot_logos):
                st.session_state.t12_foot_logos[i]["valor"] = st.text_input("Logo Footer", logo["valor"], key=f"t12_fl_{i}")
            for i, desc in enumerate(st.session_state.t12_foot_descs):
                st.session_state.t12_foot_descs[i]["valor"] = st.text_area("Copyright e Descrição", desc["valor"], key=f"t12_fd_{i}")
