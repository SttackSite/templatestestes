...
def _init():
    defaults = {
        ...
        # Projetos (Grid Bento 3D)
        "t33_proj_headers": [{"valor": "TRABALHOS EM DESTAQUE"}],
        "t33_proj_items": [
            {
                "tag": "THREE.JS JOURNEY",
                "titulo": "O curso definitivo de Three.js",
                "desc": "Aprenda a criar mundos 3D incríveis para a web partindo do zero absoluto.",
                "img_bg": "#222",
                "btn_texto": "VER CURSO",
            ...
            },
            {
                ...
                "titulo": "Oris: O Relógio Interativo",
                "desc": "Uma experiência imersiva para explorar cada detalhe da mecânica de luxo em tempo real.",
                "img_bg": "#fff",
                "btn_texto": "VER PROJETO",
                "btn_url": "https://www.google.com/"
            }
        ],
        # Seção Sobre
    ...
    with col_form:
        ...
        with st.container(height=720, border=False):
            ...
            for i, cor in enumerate(st.session_state.t33_cores):
                ...
                with c1: st.session_state.t33_cores[i]["nome"] = st.text_input("Nome", cor["nome"], key=f"t33_cor_n_{i}", label_visibility="collapsed")
                with c2: st.session_state.t33_cores[i]["valor"] = st.color_picker("Cor", cor["valor"], key=f"t33_cor_v_{i}", label_visibility="collapsed")
                with c3:
            ...
            for i, item in enumerate(st.session_state.t33_nav_logos):
                st.session_state.t33_nav_logos[i]["texto"] = st.text_input("Logo (Iniciais)", item["texto"], key=f"t33_nav_l_t_{i}")
            
            st.caption("Links do Menu")
            for i, link in enumerate(st.session_state.t33_nav_links):
                c1, c2, c3 = st.columns([4, 5, 1])
                with c1: st.session_state.t33_nav_links[i]["texto"] = st.text_input("Texto", link["texto"], key=f"t33_navl_t_{i}", label_visibility="collapsed")
                with c2: st.session_state.t33_nav_links[i]["url"] = st.text_input("URL", link["url"], key=f"t33_navl_u_{i}", label_visibility="collapsed")
                with c3:
            ...
            st.caption("Link Destaque (Dê o Play)")
            for i, cta in enumerate(st.session_state.t33_nav_ctas):
                c1, c2, c3 = st.columns([4, 5, 1])
                with c1: st.session_state.t33_nav_ctas[i]["texto"] = st.text_input("Texto", cta["texto"], key=f"t33_ncta_t_{i}", label_visibility="collapsed")
                with c2: st.session_state.t33_nav_ctas[i]["url"] = st.text_input("URL", cta["url"], key=f"t33_ncta_u_{i}", label_visibility="collapsed")
                with c3:
            ...
            for i, t in enumerate(st.session_state.t33_hero_titulos):
                st.session_state.t33_hero_titulos[i]["valor"] = st.text_area("Título Hero (HTML)", t["valor"], key=f"t33_h_t_{i}")
            for i, stt in enumerate(st.session_state.t33_hero_subtitulos):
                st.session_state.t33_hero_subtitulos[i]["valor"] = st.text_input("Subtítulo Hero", stt["valor"], key=f"t33_h_st_{i}")
            for i, btn in enumerate(st.session_state.t33_hero_btns):
                c1, c2 = st.columns(2)
                with c1: st.session_state.t33_hero_btns[i]["texto"] = st.text_input("Texto Botão Hero", btn["texto"], key=f"t33_hb_t_{i}")
                with c2: st.session_state.t33_hero_btns[i]["url"] = st.text_input("URL Botão Hero", btn["url"], key=f"t33_hb_u_{i}")
            for i, sim in enumerate(st.session_state.t33_hero_simulacoes):
                st.session_state.t33_hero_simulacoes[i]["img_url"] = st.text_input("URL Imagem Simulação", sim["img_url"], key=f"t33_h_s_i_{i}")
                st.session_state.t33_hero_simulacoes[i]["texto"] = st.text_input("Texto Overlay Simulação", sim["texto"], key=f"t33_h_s_t_{i}")
            ...
            for i, ph in enumerate(st.session_state.t33_proj_headers):
                st.session_state.t33_proj_headers[i]["valor"] = st.text_input("Título Seção Projetos", ph["valor"], key=f"t33_p_h_{i}")
            
            for i, item in enumerate(st.session_state.t33_proj_items):
                with st.expander(f"Projeto {i+1}: {item['titulo']}"):
                    st.session_state.t33_proj_items[i]["tag"] = st.text_input("Tag", item["tag"], key=f"t33_pi_tag_{i}")
                    st.session_state.t33_proj_items[i]["titulo"] = st.text_input("Título", item["titulo"], key=f"t33_pi_t_{i}")
                    st.session_state.t33_proj_items[i]["desc"] = st.text_area("Descrição", item["desc"], key=f"t33_pi_d_{i}")
                    st.session_state.t33_proj_items[i]["img_bg"] = st.text_input("Cor Fundo Placeholder (Hex)", item["img_bg"], key=f"t33_pi_bg_{i}")
                    st.session_state.t33_proj_items[i]["btn_texto"] = st.text_input("Texto Botão", item["btn_texto"], key=f"t33_pi_bt_{i}")
                    st.session_state.t33_proj_items[i]["btn_url"] = st.text_input("URL Botão", item["btn_url"], key=f"t33_pi_bu_{i}")
                    if len(st.session_state.t33_proj_items) > 1 and _del_btn(f"t33_pi_del_{i}", "Remover projeto"):
                        st.session_state.t33_proj_items.pop(i); st.rerun()
            if _add_btn("t33_pi_add", "＋ Adicionar projeto"):
                st.session_state.t33_proj_items.append({"tag": "NOVA TAG", "titulo": "NOVO PROJETO", "desc": "DESC", "img_bg": "#222", "btn_texto": "VER MAIS", "btn_url": "#"}); st.rerun()
            ...
            for i, stit in enumerate(st.session_state.t33_sobre_titulos):
                st.session_state.t33_sobre_titulos[i]["valor"] = st.text_input("Título Sobre", stit["valor"], key=f"t33_s_t_{i}")
            
            st.caption("Parágrafos de Bio")
            for i, stxt in enumerate(st.session_state.t33_sobre_textos):
                c1, c2 = st.columns([9, 1])
                with c1: st.session_state.t33_sobre_textos[i]["valor"] = st.text_area("Texto", stxt["valor"], key=f"t33_s_tx_{i}", label_visibility="collapsed")
                with c2:
            ...
            for i, sbtn in enumerate(st.session_state.t33_sobre_btns):
                c1, c2 = st.columns(2)
                with c1: st.session_state.t33_sobre_btns[i]["texto"] = st.text_input("Texto Botão Sobre", sbtn["texto"], key=f"t33_sb_t_{i}")
                with c2: st.session_state.t33_sobre_btns[i]["url"] = st.text_input("URL Botão Sobre", sbtn["url"], key=f"t33_sb_u_{i}")
            ...
            for i, ft in enumerate(st.session_state.t33_foot_titulos):
                st.session_state.t33_foot_titulos[i]["valor"] = st.text_input("Título Footer", ft["valor"], key=f"t33_f_t_{i}")
            
            st.caption("Links Sociais")
            for i, flink in enumerate(st.session_state.t33_foot_links):
                c1, c2, c3 = st.columns([4, 5, 1])
                with c1: st.session_state.t33_foot_links[i]["texto"] = st.text_input("Rede", flink["texto"], key=f"t33_fl_t_{i}", label_visibility="collapsed")
                with c2: st.session_state.t33_foot_links[i]["url"] = st.text_input("URL/Email", flink["url"], key=f"t33_fl_u_{i}", label_visibility="collapsed")
                with c3:
            ...
            for i, fcp in enumerate(st.session_state.t33_foot_copys):
                st.session_state.t33_foot_copys[i]["valor"] = st.text_input("Copyright", fcp["valor"], key=f"t33_fcp_{i}")
            ...
            for i, item in enumerate(st.session_state.t13_obs):
                ...
                with c2:
                    if len(st.session_state.t13_obs) > 1 and _del_btn(f"t33_obs_del_{i}"):
