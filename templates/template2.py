...
def _init():
    defaults = {
        ...
        "t2_nav_ctas": [{"texto": "Começar Agora", "url": "https://www.google.com/"}],
...
def render():
    ...
    with col_form:
        ...
        with st.container(height=720, border=False):
            ...
            for i, cor in enumerate(st.session_state.t2_cores):
                ...
                with c1:
                    st.session_state.t2_cores[i]["nome"] = st.text_input(
                        "Nome", cor["nome"], key=f"t2_cor_nome_{i}", label_visibility="collapsed", placeholder="Nome da cor")
                with c2:
            ...
            for i, item in enumerate(st.session_state.t2_logos):
                ...
                with c1:
                    st.session_state.t2_logos[i]["parte1"] = st.text_input(
                        "Parte 1", item["parte1"], key=f"t2_logo_p1_{i}", label_visibility="collapsed", placeholder="Normal")
                with c2:
                    st.session_state.t2_logos[i]["destaque"] = st.text_input(
                        "Destaque", item["destaque"], key=f"t2_logo_dest_{i}", label_visibility="collapsed", placeholder="Destaque")
                with c3:
            ...
            for i, t in enumerate(st.session_state.t2_hero_titulos):
                ...
                with c1:
                    st.session_state.t2_hero_titulos[i]["parte1"] = st.text_input(
                        "P1", t["parte1"], key=f"t2_ht_p1_{i}", label_visibility="collapsed", placeholder="Texto")
                with c2:
                    st.session_state.t2_hero_titulos[i]["destaque"] = st.text_input(
                        "Dest", t["destaque"], key=f"t2_ht_dest_{i}", label_visibility="collapsed", placeholder="Destaque")
                with c3:
                    st.session_state.t2_hero_titulos[i]["parte2"] = st.text_input(
                        "P2", t["parte2"], key=f"t2_ht_p2_{i}", label_visibility="collapsed", placeholder="Texto 2")
                with c4:
            ...
            for i, stat in enumerate(st.session_state.t2_hero_stats):
                ...
                with c1:
                    st.session_state.t2_hero_stats[i]["numero"] = st.text_input(
                        "Num", stat["numero"], key=f"t2_hstat_n_{i}", label_visibility="collapsed", placeholder="Número")
                with c2:
                    st.session_state.t2_hero_stats[i]["label"] = st.text_input(
                        "Lab", stat["label"], key=f"t2_hstat_l_{i}", label_visibility="collapsed", placeholder="Rótulo")
                with c3:
            ...
            st.caption("Botão principal  *(Texto | URL)*")
            for i, btn in enumerate(st.session_state.t2_hero_btns):
                c1, c2, c3 = st.columns([4, 4, 1])
                with c1:
                    st.session_state.t2_hero_btns[i]["texto"] = st.text_input(
                        "Txt", btn["texto"], key=f"t2_hbtn_t_{i}", label_visibility="collapsed", placeholder="Texto")
                with c2:
                    st.session_state.t2_hero_btns[i]["url"] = st.text_input(
                        "URL", btn["url"], key=f"t2_hbtn_u_{i}", label_visibility="collapsed", placeholder="URL")
                with c3:
            ...
            st.caption("Emoji/Ícone do hero  *(bloco laranja à direita)*")
            ...
            st.caption("Cards de serviço  *(Ícone | Título | Descrição)*")
            ...
            st.caption("Boxes de diferencial  *(Título | Descrição)*")
            ...
            st.caption("Cards de plano  *(Nome | Preço | Período | Vantagens | Botão)*")
            ...
            st.caption("Depoimentos  *(Texto | Autor | Cargo)*")
            for i, test in enumerate(st.session_state.t2_test_cards):
                with st.expander(f"Depoimento de {test['autor']}"):
                    st.session_state.t2_test_cards[i]["texto"] = st.text_area(
                        "Texto", test["texto"], key=f"t2_tc_x_{i}", height=80)
                    st.session_state.t2_test_cards[i]["autor"] = st.text_input(
                        "Autor", test["autor"], key=f"t2_tc_a_{i}")
                    st.session_state.t2_test_cards[i]["cargo"] = st.text_input(
                        "Cargo/Função", test["cargo"], key=f"t2_tc_c_{i}")
            ...
            for i, obs in enumerate(st.session_state.t2_obs):
                ...
                with c1:
                    st.session_state.t2_obs[i]["valor"] = st.text_area(
                        "Obs", obs["valor"], key=f"t2_obs_{i}", height=80,
                        placeholder="Notas extras...", label_visibility="collapsed")
                with c2:
