...
def _init():
    defaults = {
        ...
        "t29_nav_links": [
            {"texto": "Manifesto",    "url": "#diferenciais", "active": False},
            {"texto": "Certificação", "url": "#impacto",      "active": False},
            {"texto": "Tecnologia",   "url": "#diferenciais", "active": False},
...
def render():
    ...
    with col_form:
        ...
        with st.container(height=720, border=False):
            ...
            for i, link in enumerate(st.session_state.t29_nav_links):
                ...
                with c1:
                    st.session_state.t29_nav_links[i]["texto"] = st.text_input(
                        "Texto", link["texto"], key=f"t29_navl_t_{i}", label_visibility="collapsed", placeholder="Texto")
                with c2:
                    st.session_state.t29_nav_links[i]["url"] = st.text_input(
                        "URL", link["url"], key=f"t29_navl_u_{i}", label_visibility="collapsed", placeholder="URL")
                with c3:
            ...
            st.caption("Badge  *(texto da pílula verde acima do título)*")
            ...
            st.caption("Título principal  *(suporta HTML, ex: <span class='eco-italic'>texto</span>)*")
            for i, t in enumerate(st.session_state.t29_hero_titulos):
                c1, c2 = st.columns([9, 1])
                with c1:
                    st.session_state.t29_hero_titulos[i]["valor"] = st.text_area(
                        "Título", t["valor"], key=f"t29_h_t_{i}", height=90, label_visibility="collapsed")
                with c2:
            ...
            st.caption("Descrição  *(suporta HTML, ex: <b>negrito</b>)*")
            for i, d in enumerate(st.session_state.t29_hero_descs):
                c1, c2 = st.columns([9, 1])
                with c1:
                    st.session_state.t29_hero_descs[i]["valor"] = st.text_area(
                        "Descrição", d["valor"], key=f"t29_h_d_{i}", height=90, label_visibility="collapsed")
                with c2:
            ...
            for i, btn in enumerate(st.session_state.t29_hero_btns):
                ...
                with c1:
                    st.session_state.t29_hero_btns[i]["texto"] = st.text_input(
                        "Texto", btn["texto"], key=f"t29_hb_t_{i}", label_visibility="collapsed", placeholder="Texto")
                with c2:
                    st.session_state.t29_hero_btns[i]["url"] = st.text_input(
                        "URL", btn["url"], key=f"t29_hb_u_{i}", label_visibility="collapsed", placeholder="URL")
                with c3:
            ...
            st.caption("Cards  *(Ícone | Título | Descrição | Botão)*")
            ...
            for i, stat in enumerate(st.session_state.t29_stat_items):
                ...
                with c1:
                    st.session_state.t29_stat_items[i]["valor"] = st.text_input(
                        "Valor", stat["valor"], key=f"t29_st_v_{i}", label_visibility="collapsed", placeholder="Ex: -40%")
                with c2:
                    st.session_state.t29_stat_items[i]["label"] = st.text_input(
                        "Rótulo", stat["label"], key=f"t29_st_l_{i}", label_visibility="collapsed", placeholder="Rótulo")
                with c3:
            ...
            if _add_btn("t29_ct_d_add", "＋ Adicionar descrição"):
                st.session_state.t29_cta_descs.append({"valor": "Nova descrição."}); st.rerun()
            ...
            for i, btn in enumerate(st.session_state.t29_cta_btns):
                ...
                with c1:
                    st.session_state.t29_cta_btns[i]["texto"] = st.text_input(
                        "Texto", btn["texto"], key=f"t29_ctb_t_{i}", label_visibility="collapsed", placeholder="Texto")
                with c2:
                    st.session_state.t29_cta_btns[i]["url"] = st.text_input(
                        "URL", btn["url"], key=f"t29_ctb_u_{i}", label_visibility="collapsed", placeholder="URL")
                with c3:
            ...
            for i, item in enumerate(st.session_state.t29_obs):
                ...
                with c1:
                    st.session_state.t29_obs[i]["valor"] = st.text_area(
                        ...
                        placeholder="Ex: Usar tons ainda mais naturais...",
