    ...
    with col_form:
        ...
        with st.container(height=720, border=False):
            ...
            for i, t in enumerate(st.session_state.t5_stats_titulos):
                st.session_state.t5_stats_titulos[i]["valor"] = st.text_input("Título da Seção", t["valor"], key=f"t5_st_t_{i}")
            ...
            for i, passo in enumerate(st.session_state.t5_passos):
                with st.expander(f"Passo {passo['num']}: {passo['titulo']}"):
                    st.session_state.t5_passos[i]["num"] = st.text_input("Número", passo["num"], key=f"t5_ps_num_{i}")
                    st.session_state.t5_passos[i]["titulo"] = st.text_input("Título", passo["titulo"], key=f"t5_ps_tit_{i}")
                    st.session_state.t5_passos[i]["desc"] = st.text_area("Descrição", passo["desc"], key=f"t5_ps_desc_{i}")
            ...
            if _add_btn("t5_ps_add", "＋ Adicionar Passo"):
                st.session_state.t5_passos.append({"num": f"{len(st.session_state.t5_passos)+1:02d}", "titulo": "NOVO PASSO", "desc": "..."}); st.rerun()
            ...
            for i, preco in enumerate(st.session_state.t5_precos):
                with st.expander(f"Plano {i+1}: {preco['plano']}"):
                    ...
                    st.session_state.t5_precos[i]["features"] = st.text_area("Features (um por linha)", preco["features"], key=f"t5_pr_f_{i}")
                    st.session_state.t5_precos[i]["btn_txt"] = st.text_input("Texto Botão", preco["btn_txt"], key=f"t5_pr_bt_{i}")
                    st.session_state.t5_precos[i]["url"] = st.text_input("URL", preco["url"], key=f"t5_pr_u_{i}")
            ...
            if _add_btn("t5_pr_add", "＋ Adicionar Plano"):
                st.session_state.t5_precos.append({"plano": "NOVO PLANO", "valor": "R$ 0", "features": "Feature 1", "btn_txt": "SELECIONAR", "url": "#"}); st.rerun()
            ...
            for i, t in enumerate(st.session_state.t5_faq_titulos):
                st.session_state.t5_faq_titulos[i]["valor"] = st.text_input("Título da Seção", t["valor"], key=f"t5_faq_t_{i}")
            ...
            for i, f in enumerate(st.session_state.t5_footer):
                st.session_state.t5_footer[i]["valor"] = st.text_input("Texto do Rodapé", f["valor"], key=f"t5_ft_v_{i}")

            # OBSERVAÇÕES
            st.markdown("<div class=\"section-label\">📝 Observações</div>", unsafe_allow_html=True)
            for i, obs in enumerate(st.session_state.t5_obs):
                st.session_state.t5_obs[i]["valor"] = st.text_area("Observações (não aparece no site)", obs["valor"], key=f"t5_obs_v_{i}")

    with col_preview:
        st.markdown("<div class=\"img-caption\">Clique na imagem para abrir em tela cheia</div>", unsafe_allow_html=True)
        st.markdown(f"<a href=\"{TEMPLATE_IMAGE_URL}\" target=\"_blank\"><div class=\"template-img-wrapper\"><img src=\"{TEMPLATE_IMAGE_URL}\"></div></a>", unsafe_allow_html=True)


# Execute para testar o template individualmente
